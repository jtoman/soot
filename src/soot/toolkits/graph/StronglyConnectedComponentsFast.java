...
public class StronglyConnectedComponentsFast<N> {
	...
	protected void recurse(N v) {
		...
		for (N succ : g.getSuccsOf(v)) {
			...
			if (indexForNodeSucc == null) {
				recurse(succ);
				lowlinkForNode.put(v, lowLinkForNodeV = Math.min(lowLinkForNodeV,
	 ...
	 *         that are true components, i.e. components which have more than
	 *         one element or consists of one node that has itself as a
	 *         successor
	 */
